...
$SourceDBs = @("gpxnet", "furmanetclone", "vedbondifeclone") # trasferiti da gpxnet perchè non devono essere rinumerati 
...
# Tabelle da migrare in ordine logico
$TablesToMigrate = @(
    "MA_SaleOrd",
...
function Get-TableColumns {
    ...
    try {
        (Invoke-Sqlcmd -Query $q -ConnectionString $ConnString -ErrorAction Stop).COLUMN_NAME
    }
    catch { $null }
}
...
function Get-TablePrimaryKey {
    ...
    $q = "SELECT kcu.COLUMN_NAME `nFROM $($Database).INFORMATION_SCHEMA.TABLE_CONSTRAINTS AS tc `nJOIN $($Database).INFORMATION_SCHEMA.KEY_COLUMN_USAGE AS kcu ON tc.CONSTRAINT_NAME = kcu.CONSTRAINT_NAME AND tc.TABLE_SCHEMA = kcu.TABLE_SCHEMA AND tc.TABLE_NAME = kcu.TABLE_NAME `nWHERE tc.CONSTRAINT_TYPE = 'PRIMARY KEY' AND tc.TABLE_NAME = '$TableName' ORDER BY kcu.ORDINAL_POSITION;"
    try {
        (Invoke-Sqlcmd -Query $q -ConnectionString $ConnString -ErrorAction Stop).COLUMN_NAME
    }
    catch { $null }
...
function Get-AllForeignKeyConstraints {
    ...
    $list = ($TableNames | ForEach-Object { "'$_'" }) -join ", "
    $q = "SELECT '['+s.name+'].['+t.name+']' AS TableName, fk.name AS ConstraintName `nFROM $($Database).sys.foreign_keys fk `nJOIN $($Database).sys.tables t ON fk.parent_object_id = t.object_id `nJOIN $($Database).sys.schemas s ON t.schema_id = s.schema_id `nWHERE t.name IN ($list)"
    try { Invoke-Sqlcmd -Query $q -ConnectionString $ConnString -ErrorAction Stop }
    catch { $null }
}
...
Write-Host "AVVIO MIGRAZIONE MA_SaleOrd*" -ForegroundColor Cyan
...
try {
    ...
    # Cancella in ordine inverso per rispettare le FK (figli prima dei genitori)
    $tablesReverse = @($TablesToMigrate)
    [Array]::Reverse($tablesReverse)
    foreach ($t in $tablesReverse) {
    ...
    foreach ($table in $TablesToMigrate) {
        ...
        foreach ($src in $SourceDBs) {
            ...
            $colsInsert = (($common | ForEach-Object { "[$_]" }) -join ", ")
            $colsSelect = (($common | ForEach-Object { "S.[$_]" }) -join ", ")
            $sql = "INSERT INTO $($DestinationDB).dbo.$table ($colsInsert) SELECT $colsSelect FROM $src.dbo.$table AS S"
            if ($pkCols) {
                $conds = (($pkCols | ForEach-Object { "D.[$_] = S.[$_]" }) -join " AND ")
