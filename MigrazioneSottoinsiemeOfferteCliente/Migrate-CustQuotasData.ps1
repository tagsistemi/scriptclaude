...
Write-Host "AVVIO MIGRAZIONE MA_CustQuotas*" -ForegroundColor Cyan
...
try {
    ...
    foreach ($t in ($TablesToMigrate | Sort-Object -Descending)) { try { Invoke-Sqlcmd -Query "DELETE FROM $($DestinationDB).dbo.$t;" -ConnectionString $ConnectionString -ErrorAction Stop } catch {} }
