...
$LogFile = "e:\MigrazioneVed\Scripts\MigrazioneSottoinsiemeOrdiniFornitore\Analyze-PurchaseOrdTableSchemas_Report.txt"
# Inizializza/Pulisce il file di log all'inizio dell'esecuzione
"Report di analisi della struttura del database per MA_PurchaseOrd generato il $(Get-Date)" | Set-Content -Path $LogFile

# Tabelle da analizzare
$TablesToAnalyze = @(
    "MA_PurchaseOrd",
    "MA_PurchaseOrdDefaults",
    "MA_PurchaseOrdDetails",
    "MA_PurchaseOrdNotes",
    "MA_PurchaseOrdParameters",
...
Write-Host "Avvio dell'analisi della struttura delle tabelle per MA_PurchaseOrd..." -ForegroundColor Green
    ...
    try {
        ...
        foreach ($group in $columnsGrouped) {
            ...
            foreach ($dbOccurrence in $columnOccurrences) {
                if ($dbOccurrence.SourceDB -eq $DestinationDB) {
                    continue # Non confrontare il DB di destinazione con se stesso
                }
