...
$LogFile = "e:\MigrazioneVed\Scripts\MigrazioneSottoinsiemeLotti\Analyze-LotsTableSchemas_Report.txt"
"Report di analisi della struttura del database per il sottoinsieme LOTTI generato il $(Get-Date)" | Set-Content -Path $LogFile

# Tabelle da analizzare (LOTTI/Seriali)
$TablesToAnalyze = @(
    "MA_Lots",
    "MA_LotSerialParameters",
    "MA_LotsMonthly",
    "MA_LotsNumbers",
    "MA_LotsStoragesQty",
    "MA_LotsStoragesQtyMonthly",
    "MA_LotsTracing",
    "MA_SerialNumbers",
    "MA_TmpLotsTracing"
)
...
Write-Host "Avvio analisi struttura tabelle LOTTI..." -ForegroundColor Green
    ...
    try {
        ...
        foreach ($db in $SourceDBs) {
            ...
            if ($missingInSource) {
                ...
                Write-Host $msg -ForegroundColor Red; $msg | Add-Content -Path $LogFile
                $missingInSource | ForEach-Object { "  - $_" | Add-Content -Path $LogFile }
            }
            ...
            if ($extraInSource) {
                ...
                Write-Host $msg -ForegroundColor Magenta; $msg | Add-Content -Path $LogFile
                $extraInSource | ForEach-Object { "  - $_" | Add-Content -Path $LogFile }
            }
        }
