...
"@
   ...
   BEGIN TRY
       ...
       WHERE DocumentType = 3801189;

       -- Poi inserisci i nuovi record
       INSERT INTO TAG_CrMaps (OldId, DocumentType, NewDocId)
       SELECT 
           JobQuotationId as OldId,
           3801189 as DocumentType,
           JobQuotationId + $increment as NewDocId
       FROM IM_JobQuotations
       WHERE JobQuotationId IS NOT NULL 
...
"@
   ...
   foreach ($table in $tables) {
       ...
       SET JobQuotationId = JobQuotationId + $increment 
       WHERE JobQuotationId IS NOT NULL AND JobQuotationId <= $increment
"@
