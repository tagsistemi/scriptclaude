...
"@
   ...
   BEGIN TRY
       ...
       WHERE DocumentType = 3801291;

       -- Poi inserisci i nuovi record
       INSERT INTO TAG_CrMaps (OldId, DocumentType, NewDocId)
       SELECT 
           WPRId as OldId,
           3801291 as DocumentType,
           WPRId + $increment as NewDocId
       FROM IM_WorksProgressReport
       WHERE WPRId IS NOT NULL 
...
"@
   ...
   foreach ($table in $tables) {
       ...
       SET WPRId = WPRId + $increment 
       WHERE WPRId IS NOT NULL AND WPRId <= $increment
"@
