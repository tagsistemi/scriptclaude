...
$LogFile = "e:\MigrazioneVed\Scripts\MigrazioneSottoinsiemeArticoli\Analyze-ItemsTableSchemas_Report.txt"
"Report di analisi della struttura del database per il sottoinsieme ARTICOLI generato il $(Get-Date)" | Set-Content -Path $LogFile
...
Write-Host "Avvio analisi struttura tabelle ARTICOLI..." -ForegroundColor Green
...
"@
    ...
    try {
        ...
        if (-not $destinationSchema -or $destinationSchema.Count -eq 0) {
            ...
            # Se manca nel dest non ha senso proseguire con i confronti delle colonne
            continue
        }
        ...
        foreach ($group in $columnsGrouped) {
            ...
            foreach ($dbOccurrence in $columnOccurrences) {
                ...
                # Lunghezze (solo per tipi carattere)
                ...
                # Precisione/scala (solo per DECIMAL/NUMERIC)
