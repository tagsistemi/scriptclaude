...
$tables = @(
   ...
   "IM_WPRReferences", 
   "IM_MeasuresBooks",
   "IM_MeasuresBooksNotes",
   "IM_TmpMeasuresBooksDetails"
)
...
"@
   ...
   BEGIN TRY
       ...
       WHERE DocumentType = 3801290;

       -- Poi inserisci i nuovi record
       INSERT INTO TAG_CrMaps (OldId, DocumentType, NewDocId)
       SELECT 
           MeasuresBookId as OldId,
           3801290 as DocumentType,
           MeasuresBookId + $increment as NewDocId
       FROM IM_MeasuresBooks
       WHERE MeasuresBookId IS NOT NULL;
...
"@
   ...
   foreach ($table in $tables) {
       ...
       SET MeasuresBookId = MeasuresBookId + $increment 
       WHERE MeasuresBookId IS NOT NULL
"@
