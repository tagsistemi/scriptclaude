-- Vista [dbo].[BDE_10_Items] - Aggiornamento
-- Generato: 2026-02-23 21:30:32

-- Eliminazione vista esistente
IF EXISTS (SELECT * FROM sys.views v INNER JOIN sys.schemas s ON v.schema_id = s.schema_id WHERE s.name = N'dbo' AND v.name = N'BDE_10_Items')
BEGIN
    DROP VIEW [dbo].[BDE_10_Items]
    PRINT 'Vista [dbo].[BDE_10_Items] eliminata'
END
GO

-- Ricreazione vista
CREATE VIEW [dbo].[BDE_10_Items] AS
SELECT 
	I.Item													AS IdERP, 
	...
	-- SecondRateMaxTolerance[dbo].[BDE_05_UoMComparations]
	-- ScrapMaxTolerance
	-- CommitmentType
	-- OverproductionMode
...
FROM
	...
	left join MA_ItemsManufacturingData IMD
		on IMD.Item = I.Item
WHERE
	I.Item != ''  and I.BaseUoM!=''
GO

PRINT 'Vista [dbo].[BDE_10_Items] creata con successo'
GO
