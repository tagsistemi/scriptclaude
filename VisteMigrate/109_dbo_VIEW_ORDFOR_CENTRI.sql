-- Vista [dbo].[VIEW_ORDFOR_CENTRI] - Aggiornamento
-- Generato: 2026-02-23 21:30:40

-- Eliminazione vista esistente
IF EXISTS (SELECT * FROM sys.views v INNER JOIN sys.schemas s ON v.schema_id = s.schema_id WHERE s.name = N'dbo' AND v.name = N'VIEW_ORDFOR_CENTRI')
BEGIN
    DROP VIEW [dbo].[VIEW_ORDFOR_CENTRI]
    PRINT 'Vista [dbo].[VIEW_ORDFOR_CENTRI] eliminata'
END
GO

-- Ricreazione vista
CREATE VIEW [dbo].[VIEW_ORDFOR_CENTRI]
AS
SELECT DISTINCT 
                      ...
                      dbo.MA_PurchaseOrdSummary.TaxableAmount AS ImpOrdSum, dbo.MA_CostCenters.GroupCode, 'Note' AS Notes, 
                      dbo.MA_PurchaseOrdDetails.TaxableAmount, dbo.MA_PurchaseOrdDetails.Line, dbo.MA_PurchaseOrdSummary.ShippingCharges
FROM         dbo.MA_PurchaseOrd INNER JOIN
                      ...
                      dbo.MA_PurchaseOrd.Supplier = dbo.MA_CustSupp.CustSupp RIGHT OUTER JOIN
                      dbo.MA_PurchaseOrdDetails ON dbo.MA_PurchaseOrd.PurchaseOrdId = dbo.MA_PurchaseOrdDetails.PurchaseOrdId LEFT OUTER JOIN
                      dbo.MA_CostCenters ON dbo.MA_PurchaseOrdDetails.CostCenter = dbo.MA_CostCenters.CostCenter
WHERE     (dbo.MA_PurchaseOrdDetails.TaxableAmount <> 0)
GO

PRINT 'Vista [dbo].[VIEW_ORDFOR_CENTRI] creata con successo'
GO
