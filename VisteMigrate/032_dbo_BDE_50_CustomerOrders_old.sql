-- Vista [dbo].[BDE_50_CustomerOrders_old] - Aggiornamento
-- Generato: 2026-02-23 21:30:34

-- Eliminazione vista esistente
IF EXISTS (SELECT * FROM sys.views v INNER JOIN sys.schemas s ON v.schema_id = s.schema_id WHERE s.name = N'dbo' AND v.name = N'BDE_50_CustomerOrders_old')
BEGIN
    DROP VIEW [dbo].[BDE_50_CustomerOrders_old]
    PRINT 'Vista [dbo].[BDE_50_CustomerOrders_old] eliminata'
END
GO

-- Ricreazione vista
CREATE VIEW [dbo].[BDE_50_CustomerOrders_old]
AS SELECT	
...
PRINT 'Vista [dbo].[BDE_50_CustomerOrders_old] creata con successo'
GO
