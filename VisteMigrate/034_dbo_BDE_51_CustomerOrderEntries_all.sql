-- Vista [dbo].[BDE_51_CustomerOrderEntries_all] - Aggiornamento
-- Generato: 2026-02-23 21:30:34

-- Eliminazione vista esistente
IF EXISTS (SELECT * FROM sys.views v INNER JOIN sys.schemas s ON v.schema_id = s.schema_id WHERE s.name = N'dbo' AND v.name = N'BDE_51_CustomerOrderEntries_all')
BEGIN
    DROP VIEW [dbo].[BDE_51_CustomerOrderEntries_all]
    PRINT 'Vista [dbo].[BDE_51_CustomerOrderEntries_all] eliminata'
END
GO

-- Ricreazione vista
CREATE VIEW [dbo].[BDE_51_CustomerOrderEntries_all]
AS 
SELECT	
...
WHERE
  --SOD.Delivered=0 and 
	IT.Nature in (22413313,22413312)
	--LEFT OUTER JOIN
	--	(
	--		SELECT
	--			SaleOrdId
...
PRINT 'Vista [dbo].[BDE_51_CustomerOrderEntries_all] creata con successo'
GO
