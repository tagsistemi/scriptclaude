...
# Tabelle da migrare (solo DDT). Genitore prima del figlio
$TablesToMigrate = @(
    "MA_SaleDoc",
    "MA_SaleDocDetail",
...
Write-Host "AVVIO MIGRAZIONE TABELLE DDT" -ForegroundColor Cyan
...
try {
    ...
    # Pulisce destinazione rispettando dipendenze (figlio prima, poi genitore) SOLO per DocumentType = 3407873
    foreach ($t in ($TablesToMigrate | Sort-Object -Descending)) {
        try { Invoke-Sqlcmd -Query "DELETE FROM $($DestinationDB).dbo.$t WHERE DocumentType = 3407873;" -ConnectionString $ConnectionString -ErrorAction Stop } catch {}
    }
    ...
    foreach ($table in $TablesToMigrate) {
        ...
        foreach ($src in $SourceDBs) {
            ...
            # Filtra solo i record con DocumentType = 3407873
            $sql = "INSERT INTO $($DestinationDB).dbo.$table ($colsInsert) SELECT $colsSelect FROM $src.dbo.$table AS S WHERE S.DocumentType = 3407873"
            if ($pkCols) { $conds = $pkCols | ForEach-Object { "D.[$_] = S.[$_]" } | Join-String -Separator " AND "; if ($conds) { $sql += " AND NOT EXISTS (SELECT 1 FROM $($DestinationDB).dbo.$table AS D WHERE $conds)" } }
            $sql += ";"
...
foreach ($table in $TablesToMigrate) {
    $destCount = (Invoke-Sqlcmd -Query "SELECT COUNT(*) AS C FROM $($DestinationDB).dbo.$table WHERE DocumentType = 3407873;" -ConnectionString $ConnectionString -ErrorAction Stop).C
    $srcTotal = 0
    foreach ($src in $SourceDBs) {
        ...
        if ($exists -gt 0) { $srcTotal += (Invoke-Sqlcmd -Query "SELECT COUNT(*) AS C FROM $src.dbo.$table WHERE DocumentType = 3407873;" -ConnectionString $ConnectionString -ErrorAction Stop).C }
    }
    Write-Host "Tabella $table -> Sorgenti (DocumentType=3407873): $srcTotal, Destinazione: $destCount" -ForegroundColor White
}
