...
# Definizione delle tabelle da aggiornare (ordine importante: prima parent table, poi child tables)
$tables = @(
    "IM_QuotationRequests",
    "IM_QuotaRequestsNotes",
    "IM_QuotaRequestsReferences",
    "IM_QuotaRequestsDocuments"
)
...
"@
   ...
   BEGIN TRY
       ...
       WHERE DocumentType = 3801310;

       -- Poi inserisci i nuovi record
       INSERT INTO TAG_CrMaps (OldId, DocumentType, NewDocId)
       SELECT 
           QuotationRequestId as OldId,
           3801310 as DocumentType,
           QuotationRequestId + $increment as NewDocId
       FROM IM_QuotationRequests
       WHERE QuotationRequestId IS NOT NULL 
       COMMIT TRANSACTION;
   END TRY
   BEGIN CATCH
       IF @@TRANCOUNT > 0
...
"@
    ...
    foreach ($table in $tables) {
       ...
       SET QuotationRequestId = QuotationRequestId + $increment 
       WHERE QuotationRequestId IS NOT NULL AND QuotationRequestId <= $increment
"@
